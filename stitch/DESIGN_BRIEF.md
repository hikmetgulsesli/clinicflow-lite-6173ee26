...
- ClinicFlow Lite turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a compact browser clinic appointment triage app called ClinicFlow Lite. It should help a front desk operator review today's appointment queue, flag urgent patient messages, assign a room, and update visit status. Use Vite React web app. Keep scope small: dashboard, triage queue, patient detail drawer,…
...
- Patient Operations - ClinicFlow Lite
- Patient Editor - ClinicFlow Lite
- Queue and Status Management - ClinicFlow Lite
- Insights - ClinicFlow Lite
- Settings and Preferences - ClinicFlow Lite
- Empty and Error Recovery - ClinicFlow Lite

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Patient Operations - ClinicFlow Lite
- surface_id: SURF_PATIENT_OPERATIONS
- unique_canvas_caption: Patient Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Patient data.
...
- data_entities: Patient, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Patient Editor - ClinicFlow Lite
- surface_id: SURF_PATIENT_EDITOR
- unique_canvas_caption: Patient Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Patient changes.
...
- data_entities: Patient, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_PATIENT_OPERATIONS -> Save returns to SURF_PATIENT_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Queue and Status Management - ClinicFlow Lite
...
- purpose: Help users organize Patient work by queue, status, and SLA state when those signals are part of the requested product.
...
- entry_exit_rules: SURF_PATIENT_OPERATIONS -> Status changes keep the user in the same Patient context and expose recoverable failure feedback.
- design_guidance: Represent operational state clearly; do not flatten queue/SLA work into decorative metrics.

SCREEN_SPEC_4:
- exact_screen_title: Insights - ClinicFlow Lite
...
- purpose: Show useful summaries, trends, and status signals derived from Patient data without becoming a separate analytics product.
- required_content: small metrics, recent activity, state distribution, actionable follow-up hints, empty/error state.
- data_entities: Patient, ActivityEvent
- visible_actions: ACT_FILTER_INSIGHTS as context_menu, ACT_EXPORT_SUMMARY as secondary_button
- entry_exit_rules: SURF_PATIENT_OPERATIONS -> No external BI/admin area; returns to SURF_PATIENT_OPERATIONS.
- design_guidance: Keep insight content project-relevant and compact; no generic charts unrelated to the requested domain.

SCREEN_SPEC_5:
- exact_screen_title: Settings and Preferences - ClinicFlow Lite
...
- purpose: Let users adjust Patient workflow preferences, saved filters, defaults, or product settings requested by the task.
...
- entry_exit_rules: SURF_PATIENT_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
- design_guidance: Settings must support the requested workflow only; do not invent unrelated profile or billing areas.

SCREEN_SPEC_6:
- exact_screen_title: Empty and Error Recovery - ClinicFlow Lite
...
- purpose: Keep the Patient workflow usable when data is missing, filtered away, loading, failed, or corrupt.
...
- entry_exit_rules: SURF_PATIENT_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Patient workflow and preserves unrelated state.
...
## PRODUCT_SURFACES
1. SURF_PATIENT_OPERATIONS - Patient Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Patient data.
   Data: Patient, ActivityEvent, Preference
...
2. SURF_PATIENT_EDITOR - Patient Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Patient changes.
   Data: Patient, ValidationError
   ...
   Entry/exit: SURF_PATIENT_OPERATIONS -> Save returns to SURF_PATIENT_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
...
3. SURF_QUEUE_AND_STATUS_MANAGEMENT - Queue and Status Management
   ...
   Entry/exit: SURF_PATIENT_OPERATIONS -> Status changes keep the user in the same Patient context and expose recoverable failure feedback.
...
4. SURF_INSIGHTS - Insights
   ...
   Entry/exit: SURF_PATIENT_OPERATIONS -> No external BI/admin area; returns to SURF_PATIENT_OPERATIONS.
...
5. SURF_SETTINGS_AND_PREFERENCES - Settings and Preferences
   ...
   Entry/exit: SURF_PATIENT_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
...
6. SURF_EMPTY_AND_ERROR_RECOVERY - Empty and Error Recovery
   ...
   Entry/exit: SURF_PATIENT_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Patient workflow and preserves unrelated state.
